...
def test_add_expense_form_renders_required_fields_and_all_categories(client):
    ...
    standard_categories = ['Bills', 'Entertainment', 'Food', 'Health', 'Other', 'Shopping', 'Transport']
...
def test_add_expense_all_seven_categories_supported(client):
    ...
    for idx, category in enumerate(categories, start=1):
        ...
        with app.app_context():
            db = get_db()
            row = db.execute("SELECT * FROM expenses WHERE description = ?", (desc,)).fetchone()
            assert row is not None
...
def test_add_expense_whitespace_trimming(client):
    ...
    with app.app_context():
        db = get_db()
        row = db.execute("SELECT * FROM expenses WHERE description = 'Trimmed Lunch'").fetchone()
        assert row is not None
...
def test_add_expense_minimum_positive_float_amount(client):
    ...
    with app.app_context():
        db = get_db()
        row = db.execute("SELECT * FROM expenses WHERE description = 'Smallest penny transaction'").fetchone()
        assert row is not None
...
def test_add_expense_large_amount(client):
    ...
    with app.app_context():
        db = get_db()
        row = db.execute("SELECT * FROM expenses WHERE description = 'Annual Commercial Rent'").fetchone()
        assert row is not None
...
def test_add_expense_invalid_date_format(client):
    ...
    invalid_dates = ['2026/03/20', '20-03-2026', 'not-a-date', '2026-02-31', '2026-13-01']
...
def test_add_expense_validation_failure_does_not_insert_db_record(client):
    ...
    with app.app_context():
        db = get_db()
        count_before = db.execute("SELECT COUNT(*) as cnt FROM expenses").fetchone()['cnt']
    ...
    with app.app_context():
        db = get_db()
        count_after = db.execute("SELECT COUNT(*) as cnt FROM expenses").fetchone()['cnt']
...
def test_add_expense_direct_db_verification_and_user_link(client):
    ...
    with app.app_context():
        db = get_db()
        user = db.execute("SELECT id FROM users WHERE email = 'linkeduser@spendly.com'").fetchone()
        user_id = user['id']
...
def test_add_expense_updates_dashboard_and_profile_totals(client):
    ...
    dash_init = client.get('/dashboard')
...
def test_add_expense_multi_user_isolation(client):
    ...
    beta_dash = client.get('/dashboard')
