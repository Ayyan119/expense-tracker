...
def test_login_missing_fields(client):
    ...
    response = client.post(
        "/login", data={"email": "", "password": "demo123"}, follow_redirects=True
    )
    assert response.status_code == 200
    ...
    response = client.post(
        "/login",
        data={"email": "demo@spendly.com", "password": ""},
        follow_redirects=True,
    )
    assert response.status_code == 200
    assert b"Please provide email and password." in response.data
...
def test_login_nonexistent_email(client):
    ...
    response = client.post(
        "/login",
        data={"email": "nonexistent@spendly.com", "password": "demo123"},
        follow_redirects=True,
    )
    assert response.status_code == 200
    assert b"Invalid email or password." in response.data
...
def test_seed_db_idempotency(client):
    ...
    with app.app_context():
        ...
        expense_count = db.execute("SELECT COUNT(*) as count FROM expenses").fetchone()[
            "count"
        ]
        assert user_count == 1
